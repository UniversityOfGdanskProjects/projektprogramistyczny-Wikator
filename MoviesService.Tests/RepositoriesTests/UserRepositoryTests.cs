...
public class UserRepositoryTests
{
    ...
    public UserRepositoryTests(TestDatabaseSetup testDatabase)
    {
        ...
        // language=Cypher
        const string query = "CREATE (u:User { id: $userId, name: 'TestUser', email: 'test@gmail.com', lastActive: datetime(), activityScore: 1, role: 'User' })";
    ...
    public async Task GetUserActiveTodayCount_ShouldReturnCountOfUsersActiveToday_WithUserNotActiveToday()
    {
        ...
        // language=Cypher
        const string query = "CREATE (u:User { id: apoc.create.uuid(), name: 'TestUser', email: 'test2@gmail.com', lastActive: datetime({epochSeconds: $yesterdayEpoch}), activityScore: 1, role: 'User' })";
    ...
    public async Task UserExistsAsync_ShouldReturnFalse_WhenUserDoesNotExist()
    {
        ...
        // Act
        var result = await session.ExecuteReadAsync(async tx => await userRepository.UserExistsAsync(tx, Guid.NewGuid()));
