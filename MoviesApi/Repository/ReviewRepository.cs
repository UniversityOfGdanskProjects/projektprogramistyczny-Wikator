...
public class ReviewRepository(IDriver driver) : Repository(driver), IReviewRepository
{
    public async Task<bool> AddReview(int userId, UpsertReviewDto reviewDto)
    {
        ...
        try
        {
            return await session.ExecuteWriteAsync(async tx => await AddReview(tx, userId, reviewDto));
        }
        catch (Exception e)
        {
            Console.WriteLine(e);
    ...
    private static async Task<bool> AddReview(IAsyncQueryRunner tx, int userId, UpsertReviewDto reviewDto)
    {
        ...
        var reviewResult = await tx.RunAsync(query, new {userId, movieId = reviewDto.MovieId, score = reviewDto.Score});
